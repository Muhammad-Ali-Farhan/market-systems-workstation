...
# These imports intentionally occur after prepare_native_runtime(), which adds
# the packaged native extension location to sys.path.
...
def verify_native_l2() -> str:
    ...
    return (
        f"update={synchronizer.last_update_id} | "
        f"hash={synchronizer.state_hash}"
    )
...
def verify_feature_parity() -> str:
    ...
    records["bid_volume"] = rng.integers(
        100_000,
        2_000_000,
        size=count,
        dtype=np.uint32,
    )
    records["ask_volume"] = rng.integers(
        100_000,
        2_000_000,
        size=count,
        dtype=np.uint32,
    ...
    matrix = np.vstack(
        [online[int(index)] for index in offline.event_index]
    )
...
check(
    ...
    lambda: "OpenSSL and simdjson linked statically",
)
...
check(
    "market_ui package",
    lambda: __import__("market_ui.app") and "all modules importable",
)
